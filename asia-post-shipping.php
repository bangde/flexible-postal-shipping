                ...
                public function generate_rules_table_html( $key, $data ) {
                    ...
                    jQuery(document).ready(function($) {
                        ...
                        function checkExchangeWarning() {
                            // Robust selector: finds element ending with ID suffix
                            var $carrierField = $('select[id$="carrier_brand"]');
                            var $rateField = $('input[id$="exchange_rate"]');
                            
                            var carrier = $carrierField.val();
                            var rate = parseFloat($rateField.val());
                            
                            // Safety check: if fields missing, abort
                            if (!carrier || $rateField.length === 0) return;
                        ...
                        // Use robust selectors here as well
                        $(document).on('change', 'select[id$="carrier_brand"]', checkExchangeWarning);
                        $(document).on('keyup change', 'input[id$="exchange_rate"]', checkExchangeWarning);
